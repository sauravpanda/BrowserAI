...
class FileResponse {
    ...
    updateContentType() {
        ...
        this.headers.set('content-type', CONTENT_TYPE_MAP[extension as keyof typeof CONTENT_TYPE_MAP] ?? 'application/octet-stream');
    }
...
function handleError(status: number, remoteURL: string, fatal: boolean) {
    ...
    const message = ERROR_MAPPING[status as keyof typeof ERROR_MAPPING] ?? `Error (${status}) occurred while trying to load file`;
 ...
 */
export async function getModelFile(path_or_repo_id: string, filename: string, fatal = true, options: PretrainedOptions = {}) {
 ...
 */
export async function readResponse(response: Response, progress_callback: (data: { progress: number, loaded: number, total: number }) => void) {
