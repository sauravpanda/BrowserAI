...
function reshape(data: DataArray, dimensions: number[]) {
    ...
    for (let i = dimensions.length - 1; i >= 0; i--) {
        reshapedArray = (reshapedArray as any[]).reduce((acc: any[], val: any) => {
 ...
 */
export function interpolate(input: Tensor, [out_height, out_width]: number[], mode: string = 'bilinear', align_corners: boolean = false) {
 ...
 */
export async function slice(data: Tensor, starts: number[], ends: number[], axes: number[], steps: number[] | null = null) {
 ...
 * @returns {Tensor} The normalized tensor.
 */
export function layer_norm(input: Tensor, normalized_shape: number[], {
    eps = 1e-5,
} = {}) {
    ...
    if (dim === null) {
        ...
        const std = Math.sqrt(
            Array.prototype.reduce.call(
                inputData, ((a: number, b: number) => a + (b - mean) ** 2) as any, 0) as number / (inputData.length - correction)
            );

        const meanTensor = new Tensor(input.type as DataType, [mean], [/* scalar */]);
        const stdTensor = new Tensor(input.type as DataType, [std], [/* scalar */]);
