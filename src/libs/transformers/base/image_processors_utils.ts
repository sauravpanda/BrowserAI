...
function enforce_size_divisibility(size: [number, number], divisor: number) {
    return [
        Math.max(Math.floor(size[0] / divisor), 1) * divisor,
        Math.max(Math.floor(size[1] / divisor), 1) * divisor
    ];
}
 ...
 */
export function post_process_semantic_segmentation(
    outputs: any, 
    target_sizes: [number, number][] | null = null
) {
...
export class ImageProcessor extends Callable {
    ...
    constructor(config: ImageProcessorConfig) {
        ...
        if (this.do_pad && !this.pad_size && this.size && 
