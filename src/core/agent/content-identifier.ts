...
const PLATFORM_PATTERNS: Record<string, PlatformPatterns> = {
  linkedin: {
    selectors: ['[data-test-id]', '.profile-section', '.feed-shared-update-v2', '.scaffold-layout__main'],
    classPatterns: [/artdeco-card/, /profile-/, /feed-shared/, /scaffold-layout/],
    contentRules: [
      {
        type: 'profile',
  ...
  twitter: {
    selectors: ['[data-testid]', '.tweet', '.profile-timeline', '[role="article"]'],
    classPatterns: [/css-\d+/, /tweet-/, /timeline-/],
    contentRules: [
      {
        type: 'profile',
  ...
  generic: {
    ...
    classPatterns: [/main/i, /header/i, /content/i, /navigation/i],
    contentRules: [
      {
        type: 'main',
...
export class ContentSectionAnalyzer {
  ...
  private groupIntoSections(rootElement: Element): ContentSection[] {
    ...
    const ungroupedElements = Array.from(rootElement.querySelectorAll('*')).filter(
      (el) => !this.isElementInSections(el, sections),
    );
  ...
  private evaluateTaskRelevance(content: string, task: string): number {
    ...
    const matchingKeywords = taskKeywords.filter((keyword) => normalizedContent.includes(keyword));
  ...
  private determineSectionType(element: Element): ContentSection['type'] {
    ...
    for (const rule of this.patterns.contentRules) {
      if (rule.patterns.some((pattern) => textContent.includes(pattern) || className.toLowerCase().includes(pattern))) {
        return rule.type;
      }
    }
  ...
  private calculateDOMDistance(el1: Element, el2: Element): number {
    ...
    while (commonAncestorIndex < maxLength && path1[commonAncestorIndex] === path2[commonAncestorIndex]) {
      commonAncestorIndex++;
    }

    return path1.length - commonAncestorIndex + (path2.length - commonAncestorIndex);
  }
  ...
  private generateSectionSummary(sections: ContentSection[]): string {
    ...
    return relevantSections
      .map(
        (section) =>
          `${section.type.toUpperCase()}: ${section.content.substring(0, 100)}${
