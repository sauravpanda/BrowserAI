export class HTMLCleaner {
  ...
  cleanForInteractive(html: string): string {
    ...
    const interactiveRoles = new Set(['button', 'link', 'checkbox', 'radio', 'tab', 'menuitem', 'option', 'switch']);
    ...
    const processElement = (element: Element) => {
      ...
      if (interactiveElements.has(tagName) || (role && interactiveRoles.has(role))) {
  ...
  private calculateSimilarity(str1: string, str2: string): number {
    ...
    if (len1 > 100 || len2 > 100) {
      ...
      const compareStart = this.calculateLevenshteinSimilarity(str1.substring(0, 50), str2.substring(0, 50));
  ...
  cleanWithElementIDs(
    ...
    const createSelector = (element: Element): string => {
      ...
      if (element.id && !/\s/.test(element.id)) {
        // Ensure ID doesn't contain spaces
    ...
    const processNode = (node: Node): string => {
      ...
      // Paragraphs
        ...
        if (trimmedContent && trimmedContent.length > 10 && !processedTextContent.has(trimmedContent)) {
          // Add length check and deduplication
    ...
    const findLabelText = (element: Element): string | null => {
      ...
      if (commonParent) {
        ...
        if (label && !label.hasAttribute('for')) {
          // If label doesn't have 'for', assume it's for this input
          return label.textContent?.trim() || null;
        }
      }
