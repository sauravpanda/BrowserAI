    ...
    try {
      ...
      if (isExtension) {
        ...
        try {
          ...
          const result = await Tesseract.recognize(imageUrl, options.language || 'eng', {
 ...
 */
export async function extractTextFromImage(file: File, options: ImageParseOptions = {}): Promise<string> {
